...
- 자바의 어노테이션 프로세서라는 기능을 이용하여 컴파일 시점에 생성자 코드를 자동으로 생성
- 생성자를 딱 1개 두고 `@Autowired`를 생략하는 방법 사용

### 5. 조회 빈이 2개 이상 - 문제
- `@Autowired` : 타입으로 조회
- 타입으로 조회 시 선택된 빈이 2개 이상일 때 문제 발생
- 하위 타입으로 지정 가능 but DIP 위배하고 유연성이 떨어짐
...
- `@Primary` 사용
  - 우선순위를 정하는 방법
  - 여러 빈 매칭시 `@Primary`가 우선권을 가짐

### 7. 어노테이션 직접 만들기

### 8. 조회한 빈이 모두 필요할 때, List, Map
- Map : map의 키에 스프링 빈의 이름을 넣어주고, 그 값으로 타입으로 조회한 모든 스프링 빈을 담아줌
- List : 타입으로 조회한 모든 스프링 빈을 담아줌
- 해당하는 타입의 스프링 빈이 없으면 빈 컬렉션이나 Map 주입

### 9. 자동, 수동의 올바른 실무 운영 기준
- 자동 빈 등록 : OCP, DIP를 지킬 수 있음
- 수동 빈 등록
  - 업무 로직 빈 : 웹을 지원하는 컨트롤러, 핵심 비즈니스 로직이 있는 서비스, 데이터 계층의 로직을 처리하는 리포지토리, 비즈니스 요구사항을 개발할 때 추가되거나 변경
  - 기술 지원 빈 : 기술적인 문제나 공통 관심사(AOP)를 처리할 때 주로 사용, 데이터베이스 연결, 공통 로그 처리 등
...
### 1. 빈 생명주기 콜백 시작
- 객체 생성 -> 의존관계 주입
- 의존관계 주입 완료 -> 스프링 빈에게 콜백 메서드를 통해 초기화 시점을 알려줌
- 스프링 컨테이너 생성 -> 스프링 빈 생성 -> 의존관계 주입 -> 초기화 콜백 -> 사용 -> 소멸전 콜백 -> 스프링 종료
- 초기화 콜백 : 빈 생성, 빈 의존관계 주입이 완료된 후 호출
...
- 설정 정보 사용 특징
  ...
  - 코드가 아니라 설정 정보 사용 -> 코드를 고칠 수 없는 외부 라이브러리에도 초기회, 종료 메서드 적용 가능
- 종료 메서드 추론
  - 라이브러리 -> 대부분 `close`, `shutdown` 이라는 이름의 종료 메서드 사용
  - `destroyMethod` : 기본값이 `(inferred)`(추론)으로 등록
  - `close`, `shutdown`라는 이름 메서드 자동 호출 -> 종료 메서드 추론해서 호출
  - 직접 스프링 빈으로 등록 -> 종료 메서드 따로 적어주지 않아도 잘 동작함
  - `destroyMethod=""` 빈 공백 지정 : 추론 기능 사용 X
...
- 컴포넌트 스캔과 잘 어울림
- 외부 라이브러리에 적용 불가 -> `@Bean` 사용

## 9. 빈 스코프

### 1. 빈 스코프란?
- 싱글톤 : 기본 스코프, 스프링 컨테이너의 시작과 종료까지 유지되는 가장 넓은 범위의 스코프
- 프로토타입 : 스프링 컨테이너는 프로토타입 빈의 생성과 의존관계 주입까지만 관여, 더는 관리하지 않는 매우 짧은 범위의 스코프
- 웹 관련 스코프
...
- 스프링 컨테이너 -> 프로토타입 빈 생성, 의존관계 주입, 초기화까지만 처리
- 프로토타입 빈을 관리할 책임 : 프로토타입 빈을 받읕 클라이언트가 보유
- 스프링 컨테이너에서 빈을 조회할 때 생성, 초기화 메서드도 실행

### 3. 프로토타입 스코프 - 싱글톤 빈과 함께 사용시 문제점
- 일반적으로 싱글톤 빈 사용 -> 싱글톤 빈이 프로토타입 빈 사용
- 싱글톤 빈 생성 시점에만 의존관계 주입 방음 -> 프로토타입 빈 새로 생성 but 싱글톤 빈과 함께 유지

### 4. 프로토타입 스코프 - 싱글톤 빈과 함께 사용 시 Provider로 문제 해결
- `ac.getBean()` : 항상 새로은 프로토타입 빈 생성
- DL(Dependency Lookup, 의존관계 조회) : 의존관계를 외부에서 주입받는 게 아니라 직접 필요한 의존관계를 찾는 것
- 스프링 애플리케이션 컨텍스트 전체 주입 -> 스프링 컨테이너에 종속적인 코드 -> 단위 테스트 어려움
- `ObjectProvider` : 지정한 빈을 컨테이너에서 대신 찾아주는 DL 서비스 제공
- `ObjectFactory` : `ObjectProvider`의 상위 인터페이스, 기능 단순, 별도 라이브러리 필요 없음, 스프링 의존
- JSR-330 Provider : 자바 표준, `jakarta.inject.Provider`, 별도 라이브러리 필요 없음, 자바 표준
- 프로토타입 빈 -> 매번 사용할 때마다 의존관계 주입이 완료된 새로운 객체가 필요할 때 사용

### 5. 웹 스코프
- 웹 환경에서만 동작
- 스프링이 해딩 스코프의 종료시점까지 관리 -> 종료 메서드 호출
...
- 가짜 프록시 객체 : 실제 요청이 오면 그때 내부에서 실제 빈을 요청하는 위임 로직 보유
- 프록시 객체 : 실제 요청을 받아주고, 내부에서 실제 빈에게 위임하는 래퍼